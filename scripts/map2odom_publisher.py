...
class Map2OdomPublisher:
    ...
    def spin(self):
        ...
        self.broadcaster.sendTransform(pos, quat, rospy.Time.now(), self.odom_frame, 'map')


def main():
    rospy.init_node('map2odom_publisher')
    odom_frame = rospy.get_param('odom_frame', 'mapping_odom')
    node = Map2OdomPublisher(odom_frame=odom_frame)
